...
**查询元素**
```java
forEach(); // 遍历集合中的元素，需要使用lamda表达式
size();  // 返回此集合中的元素数
使用(迭代器)Iteator<T>中的 iterator()查询
	hasNext(); //判断光标后面是否有更多的元素
	next(); // 获得光标后面的元素内容
```
**判断元素**
```java
...
System.out.println(linkedList.peekFirst()); // 获取第一个元素	 列表为空，则返回 null 
System.out.println(linkedList.peekLast());// 获取最后一个元素 列表为空，则返回 null 
```
```java
删除元素
System.out.println(linkedList.remove());//删除第一个元素
System.out.println(linkedList.removeFirst()); //删除第一个元素
System.out.println(linkedList.removeLast()); //删除最后一个元素
System.out.println(linkedList.remove("hello")); // 删除“hello”元素

System.out.println(linkedList.poll()); // 获取并删除第一个元素
System.out.println(linkedList.pollFirst());// 获取并删除第一个元素 列表为空，则返回 null
System.out.println(linkedList.pollLast());// 获取并删除最后一个元素 列表为空，则返回 null 
```
**ArrayList**
```java
初始化容量10
...
区别：
1. **底层的数据结构**
   **HashSet** 是**哈希表(哈希算法)维护**
   **LinkedHashSet** 是由**哈希表+双向链表维护**   元素是**有序**的 (插入顺序和 输出顺序是一致的)
   **TreeSet**  是由**二叉树(红黑树)进行维护**   元素是**有序的**(按照自然顺序进行排列   要求集合元素数据类型必须有比较器 (实现 Comparable))
2. **元素是否可以为null**
   HashSet/LinkedHashSet 元素**可以为null**
   TreeSet 元素**不允许为null**
3. **线程是否安全**
...
System.out.println("获得文件大小字节数:" + file.length());
System.out.println("修改的时间:" + file.lastModified());// 毫秒数
System.out.println(new Date(file.lastModified()));
System.out.println("文件的绝对路径:" + file.getAbsolutePath());
System.out.println("文件的相对路径:" + file.getPath());
System.out.println("文件是否可读:" + file.canWrite());
file.delete(); // 删除文件
System.out.println("获得目录名:" + file.getName());
System.out.println("修改的时间:" + file.lastModified());// 毫秒数
System.out.println("目录的绝对路径:" + file.getAbsolutePath());
System.out.println("目录的相对路径:" + file.getPath());
```
##### 字节操作
...
try (InputStream inputStream = new FileInputStream(file);) 
{
	...
	int result = inputStream.read(by, 0, 4);//off 从字节数组的第几个元素开始写入 ，len: 写入多少个元素到字节数组
	//将文件内容读入到字节数组
...
try (
	// 1.创建输出流对象
	OutputStream outputStream = new FileOutputStream(file,true);//文件不存在可以自定去创建
) {
	outputStream.write('\n');
